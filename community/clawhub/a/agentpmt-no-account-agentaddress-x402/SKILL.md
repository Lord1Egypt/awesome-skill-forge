...
- Use the exact product and action slugs from the URL. Do not re-slugify, lowercase, or change casing after choosing the URL.
...
- Canonical bytes must match Python `json.dumps(value, sort_keys=True, separators=(",", ":"), ensure_ascii=True)`.
...
function escapeNonAscii(jsonText) {
  return jsonText.replace(/[^\x00-\x7F]/g, (char) =>
    "\\u" + char.charCodeAt(0).toString(16).padStart(4, "0")
  );
}

export function canonicalJson(value) {
  return escapeNonAscii(JSON.stringify(sortJson(value)));
}
...
| 401 with `Invalid session nonce` or `Session nonce expired` | Session failed | Create a new session nonce, use a fresh `request_id`, rebuild the message, and sign again. |
| 401 with `Signature verification failed` or `Signature does not match wallet` | Signed bytes do not match the server's reconstructed message | Do not loop by only refreshing the session. Compare wallet casing, called URL versus signed path, canonical JSON bytes, `payload_hash`, request id, and message newlines. |
